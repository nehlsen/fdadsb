...
class PresetTesting
{
    public static function create($playerIdByGroup)
    {
        ...
        $groupRound->setGameMode(new GameMode(GameMode::FIRST_TO, 1));
        $groupRound->setLegMode(new LegMode(LegMode::SINGLE_OUT_25));
        ...
        $crisCrossRound->setGameMode(new GameMode(GameMode::FIRST_TO, 1));
        $crisCrossRound->setLegMode(new LegMode(LegMode::SINGLE_OUT_25));
        $setup->addRound($crisCrossRound);

        $xRound = RoundSetupAva::createReduce(); // create an AvA round with reduce input
        $xRound->setAdvance(2);
        $xRound->setGameMode(new GameMode(GameMode::FIRST_TO, 1));
        $xRound->setLegMode(new LegMode(LegMode::SINGLE_OUT_25));
        $setup->addRound($xRound);

        $yRound = RoundSetupAva::createSamePlace(); // create an AvA round with same-place input
        $yRound->setAdvance(2);
        $yRound->setGameMode(new GameMode(GameMode::FIRST_TO, 1));
        $yRound->setLegMode(new LegMode(LegMode::SINGLE_OUT_25));
